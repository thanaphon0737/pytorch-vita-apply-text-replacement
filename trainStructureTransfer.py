...
def main():
    ...
    # netShapeM.load_state_dict(torch.load(opts.structure_load))
    # netShapeM.eval()
    netShapeM.train()
    ...
    Xl, X, _, Noise = load_style_image_pair_og(opts.style_name,opts.std_name, scales, netSketch, opts.gpu)
    ...
    for epoch in range(opts.step1_epochs):
        for i in range(opts.Straining_num//opts.batchsize):
            ...
            xl, x = cropping_training_batches_og(Xl[idx], X, Noise, opts.batchsize, 
    ...
    time_path = opts.style_name.split('/')[-1]
    time_path = time_path.split('.')[0]
