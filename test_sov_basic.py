...
def main():
    ...
    final1 = combine_patch_basic(text,opts,C,ep)
    # final2 = combine_patch(text,opts,C,100,ep)
    # cv2.imshow('final1',final1)
    # cv2.imshow('final2',final2)
    # cv2.waitKey(0)
    result_filename = os.path.join(opts.result_dir, ('B-'+str(opts.pic)+ '_' + str(ep) + '_' + opts.name+'.png'))
    cv2.imwrite(result_filename,final1)
